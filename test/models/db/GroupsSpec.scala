...
class GroupsSpec extends Specification {
  ...
  "Groups" should {
    ...
    "find by where clauses" in new AutoRollback {
      val maybeFound = Groups.findBy(sqls.eq(g.groupId, 1L))
      maybeFound.isDefined should beTrue
    }
    ...
    "find all by where clauses" in new AutoRollback {
      val results = Groups.findAllBy(sqls.eq(g.groupId, 1L))
      results.size should be_>(0)
    }
    "count by where clauses" in new AutoRollback {
      val count = Groups.countBy(sqls.eq(g.groupId, 1L))
      count should be_>(0L)
    }
    "create new record" in new AutoRollback {
      val created = Groups.create(groupId = 1L, groupName = "MyString")
